...
class TugEdit(View):
	def get(self, request, tid=None):
		...
		return render(request, 'tug-edit.html', data)
	def post(self, request, tid=None):
		me, rqstPst=request.user, request.POST
		title, coordinate, description=rqstPst['title'], rqstPst['coordinate'], rqstPst['description']
		tug=Tug.objects.get(id=tid)
		#if title!=tug.title: if body != tug.body:
		tugRevised= (title is not tug.title) or (description is not tug.description)
		#tugRevised=True if title!=tug.title or body!=tug.body else False
		if tugRevised:
			tug.title=title
...
class TugDetail(View):
	def get(self, request, tid=None):
			...
			return render(request, 'tug-detail.html', {'tug':tug, 'tid':tug.id, 'iid':tug.initiator.id, 'medium':tug.media.all(), 'tuggers':tug.tuggers, 'title':tug.title.title, 'max':tug.max, 'coordinate':tug.coordinate, 'datetime':tug.datetime,'description':tug.description, 'timestamp':tug.timestamp, 'perm':perm})
