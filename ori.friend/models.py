...
from django.db.models import Model, ForeignKey, TextField, DateTimeField, Q, CASCADE
...
class FriendshipRequest(Model):
    ...
    from_user = ForeignKey(AUTH_USER_MODEL, related_name='friendship_requests_sent', on_delete=CASCADE)
    to_user = ForeignKey(AUTH_USER_MODEL, related_name='friendship_requests_received', on_delete=CASCADE)
...
class FriendshipManager(BaseUserManager):
    """ Friendship manager """
...
class Friend(Model):
    ...
    to_user = ForeignKey(AUTH_USER_MODEL, related_name='friends', on_delete=CASCADE)
    from_user = ForeignKey(AUTH_USER_MODEL, related_name='_unused_friend_relation', on_delete=CASCADE)
    created = DateTimeField(default=timezone.now)
...
class FollowingManager(BaseUserManager):
    """ Following manager """
...
class Follow(Model):
    ...
    follower = ForeignKey(AUTH_USER_MODEL, related_name='following', on_delete=CASCADE)
    followee = ForeignKey(AUTH_USER_MODEL, related_name='followers', on_delete=CASCADE)
    created = DateTimeField(default=timezone.now)
...
class BlockManager(BaseUserManager):
    """ Following manager """
...
class Block(Model):
    """ Model to represent Following relationships """
    blocker = ForeignKey(AUTH_USER_MODEL, related_name='blocking', on_delete=CASCADE)
    blocked = ForeignKey(AUTH_USER_MODEL, related_name='blockees', on_delete=CASCADE)
    created = DateTimeField(default=timezone.now)
